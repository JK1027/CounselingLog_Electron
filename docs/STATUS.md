...
* **주요 시스템 상태**: 
  ...
  - **데스크톱 셸**: Electron 32 기반 윈도우 패키징. 배포 버전 빌드 완료 및 자동 업데이트(`autoUpdater`) 연동.
  - **인쇄 시스템**: Wee 개인상담 기록지 및 Wee 집단상담 기록지 격자 양식 인쇄 지원. 대장 목록 양식 A4 규격 자동 페이징 구현.
  - **로그인 시스템**: 빌드 주입식 해시 암호('1qaz') 기반 잠금 화면(Lock Screen) 적용.

## 2. 주요 탑재 기능
* **상담 관리(CRUD)**: 개인/보호자/교원자문/의뢰/집단/또래상담의 시트별 데이터 읽기, 쓰기, 수정, 삭제 및 순번 재정렬 기능.
* **사용성 및 입력 생산성**: 
  - 한글 초성 검색 및 학번 검색 지원 커맨드 팔레트 (Ctrl+K).
  - 연속 입력 모드, 단축키 시스템(Ctrl+N, Ctrl+S, Esc, Alt+Arrow).
  - 컴팩트 모드(25% 축소 뷰), Resizable Split Pane(사이드바/에디터 크기 조절).
  - 달력 선택기(`DateInput`) 및 윤년/월별 최대 일수 유효성 검사.
* **엑셀 보안 및 동시성**:
  - `_record_id` 고유 식별자 열 숨김(Hidden) 처리.
  - openpyxl 서식 복제 및 동적 유효성 검사 범위 연장, 자동 필터 보존.
  - API 수준 트랜잭션 락 및 mtime 2초 스로틀링 레이지 캐싱.
  - `PermissionError`(파일 잠금) 발생 시 Safe Save 롤백 및 Toast 안내.

## 3. 향후 계획 및 Roadmap
- [ ] 엑셀 대용량 로딩 및 다중 쓰기 성능 지속 모니터링
- [ ] 또래상담자 등록 명단 관리 기능 웹 UI 고도화
- [ ] 데이터베이스 마이그레이션 필요성 검토 (엑셀 외 경량 DB 사용성 분석)
...
## [2026-05-24] 또래상담 명단 추가 및 저장 UX 사용성 개선 완료 (v0.2.10)

### 작업 내용
- [x] **[Frontend] 하단 폼 작성 중 직접 저장 시 자동 반영 및 검증 가드 구현 (`PeerStudentManager.jsx`)**:
  - 사용자가 하단 추가 폼에 학년, 반, 번호, 이름, 학번을 입력한 상태에서 우측의 추가(`+`) 단추를 누르지 않고 곧바로 하단의 **[명단 저장]** 버튼을 클릭했을 때의 동작을 개선.
  - **자동 추가 기능**: 모든 정보가 빈틈없이 유효하게 작성되어 있을 경우, [명단 저장] 클릭 시 자동으로 목록에 추가하여 서버에 함께 저장되도록 보완.
  - **불완전 입력 차단 및 안내**: 일부 필드만 채워졌거나 정합성 조건(학번 4자리, 학년-학번 일치 등)에 어긋나는 경우, 저장을 일시 차단하고 친근한 설명의 Toast warning 피드백(`작성 중인 새 학생 정보가 있습니다. 추가(+) 버튼을 클릭하여 목록에 등록하거나, 입력창을 비워주세요.`)을 노출하여 데이터 유실 원천 방지.
- [x] **무결성 정적 빌드 검증 완료**:
...
## [2026-05-26] 백업 파일 저장 위치 사용자 설정 기능 및 설정 모달 추가 완료 (v0.2.13)
...
- [x] **[Release] 버전 0.2.13 업데이트 및 GitHub Actions 자동 배포 트리거**:
  - `electron/package.json` 버전을 `0.2.13`로 범프하고 `v0.2.13` 릴리즈 태그를 원격 저장소에 푸시하여 배포 파이프라인 가동 완료.
...
## [2026-05-26] 엑셀 드롭다운 실시간 파싱 및 상담 구분 동적 매핑 구현 및 릴리즈 배포 완료 (v0.2.17)

### 작업 내용
- [x] **[Release] 버전 0.2.17 업데이트 및 GitHub Actions 자동 배포 트리거**:
  - `electron/package.json` 버전을 `0.2.17`로 범프하고 `v0.2.17` 릴리즈 태그를 원격 저장소에 푸시하여 배포 파이프라인 가동 완료.
...
- [x] **[Frontend] 인쇄 미리보기 레이아웃 개편 및 제목 수정**:
  ...
  - 집단상담 서식 인쇄일 경우 제목이 `Wee 집단상담 기록지`로 동적 출력되도록 구현.
- [x] **[Frontend] 상세 보고서 인적사항 격자 표 컴포넌트 신설 (`PrintSessionHeader.jsx`)**:
  ...
  - 집단상담 인쇄 시 첫째 줄을 `상담참여학생` 라벨로 단일화하고 학번에 매핑되는 학생 명단을 `홍길동(2301)` 형식으로 자동 변환해 콤마 구분 노출하도록 구현.
...
- [x] **[Frontend] 인쇄 미리보기 화면 가로 중앙 정렬 스타일 강화 및 안정화 (`PrintPreview.jsx`)**:
  - 상단 고정 제어 툴바 적용 시 화면이 좌측으로 쏠리던 레이아웃 문제를 해결하기 위해, 미리보기 본문 영역에 `flex flex-col items-center`를 적용하고 각 개별 리포트 및 대장 양식 컨테이너에 `mx-auto`를 명시하여 브라우저 및 화면 크기에 관계없이 항상 종이 양식이 가로 중앙에 오도록 레이아웃 정렬을 완벽히 복구했습니다.
- [x] **[Frontend] 상세 보고서 인쇄 서식에서 '출력일시' 삭제 (`PrintReportCard.jsx`)**:
  - 사용자의 요청에 따라 상세 보고서 기록지 서식 상단 우측에 렌더링되던 출력일시(오늘 날짜) 영역을 완전히 제거하고 관련 불필요 코드(todayStr)를 정리했습니다.
- [x] **[Frontend] 인쇄 미리보기에서 설정 모달 복귀용 뒤로가기 네비게이션 구현**:
  - `PrintPreview.jsx`의 "닫기 (ESC)" 버튼을 "뒤로 돌아가기 (ESC)"로 변경하고 `ArrowLeft` 아이콘을 추가하여 비주얼을 정돈했습니다.
  - 뒤로 가기 및 ESC 클릭 시, 이전에 사용자가 설정했던 옵션들(출력 대상, 조회 기간, 정렬 방식 등)을 `AppShell.jsx` 레벨에서 `PrintSetupModal`로 바인딩 이전함으로써 기존 입력을 그대로 보존한 채 설정 모달로 돌아갈 수 있도록 복귀 동선을 개선했습니다.
- [x] **[Frontend] 집단상담 일지 상세 보고서 서식에서 '상담참여학생' 행 삭제 (`PrintSessionHeader.jsx`)**:
  - 집단상담 기록지 출력 시 첫 번째 줄에 렌더링되던 "상담참여학생" 이름 및 학번 목록 표시 행을 삭제하여 레이아웃을 간결화했습니다. (개인상담 양식 헤더는 기존 이름/성별/학년반 행을 정상 노출하도록 유지)
- [x] **[Frontend] 인쇄용 상세 보고서 일지 서식에 '상담내용' 레이블 셀 추가 (`PrintReportCard.jsx`)**:
  - 기존에 colSpan={2}로 가득 차 있던 상담내용 상세 글 영역 왼쪽에 `"상담내용"` 구분 셀을 신설하여, 상단 상담제목 컬럼의 좌우 너비 분할 구조(`12%`: `88%`)와 완벽히 통일된 그리드를 이룩했습니다.
- [x] **[Frontend] 상세 보고서 인쇄 서식 헤더에 '학년/반/번호' 표시 추가 (`PrintSessionHeader.jsx`)**:
  - 기존의 `"학년/반"` 대신 `"학년/반/번호"`를 노출하도록 헤더 셀 라벨을 변경했습니다.
  - 학번(`studentId`) 파싱 알고리즘을 보강해 학년, 반에 이어 학생의 고유 출석 번호(예: `20번`)까지 자동으로 추출하여 `"3학년 1반 20번"` 형식으로 조립해 출력하도록 개선했습니다.
  - "학년/반/번호" 레이블이 좁은 영역 때문에 2줄로 밀려 출력되는 문제를 해결하기 위해, 이름 및 성별 내용 열의 불필요한 공백 너비를 소폭 줄이고(각각 21% -> 18%), 해당 레이블 영역의 너비를 넓힘(12% -> 17%)과 동시에 `whitespace-nowrap` 속성을 주어 완벽하게 한 줄에 잘리지 않고 표시되도록 레이아웃을 고도화했습니다.
...
## [2026-05-28] 대장(목록) 양식 '학생 상담 현황' 구조 개편

### 완료 작업
- [x] **[Frontend] 대장 양식 타이틀 동적 연도 및 상담 유형 연동 (`PrintPreview.jsx`)**:
  - `PrintPreview` 내에서 시작일(`startDate`)의 연도 또는 출력용 세션 날짜를 기반으로 학년도를 자동 추출하는 `getSchoolYear` 헬퍼를 추가했습니다.
  - 출력 필터를 파악하여 `개인상담`, `집단상담`, `전체상담` 등 적절한 대상 문자열을 노출하는 `getTypeText` 헬퍼를 추가했습니다.
  - 대장 양식 출력 타이틀(`h1`)을 `"{getSchoolYear()}학년도 학생 상담 현황({getTypeText()})"` 구조로 전면 갱신했습니다.
- [x] **[Frontend] 학생 단위 중복 제거 및 상담구분별 횟수 누적 집계 (`PrintRegisterTable.jsx`)**:
  - 전달받은 세션 목록(`printData`)을 순회하며 학생(`name` + `studentId`) 기준으로 그룹화하고, 동일한 학생 내에서 각 상담구분(`type`)별 등장 횟수를 누적 집계하는 맵-리듀스 형태의 가공 알고리즘을 구현했습니다.
  - 최초 데이터 정렬 구조(`sortBy` 옵션)를 그대로 계승하도록 그룹화 배열을 조립함으로써 학생들의 노출 정렬 순서가 흩어지지 않도록 처리했습니다.
- [x] **[Frontend] 대장 테이블 컬럼 개편 및 셀 내부 정렬선 구현 (`PrintRegisterTable.jsx`)**:
  - 기존 대장 컬럼 구성을 삭제/조정하고 `순번(8%)`, `이름(15%)`, `학년/반(15%)`, `상담구분(32%)`, `횟수(12%)`, `비고(18%)`의 6개 컬럼 체계로 개편했습니다.
  - 학번 또는 반 정보를 이용하여 `"O학년 O반"` 형식의 학년/반 데이터를 출력하고, 정보가 없는 집단상담의 경우 학년명(예: "혼합")을 깔끔히 노출하는 `formatGradeClass` 포맷터를 적용했습니다.
  - 한 학생에게 다수의 상담구분이 발생한 경우, `상담구분`과 `횟수` 셀 내부에서 각 상담별 라인이 완벽히 수평으로 일치하도록 동일한 개수의 내부 `div` 블록 및 하단 경계선(`border-b`)을 매핑해 비주얼을 깔끔하게 구축했습니다.
...
## [2026-05-28] 대장(목록) 양식 A4 용지 규격 기준 자동 페이지 분할(Pagination) 구현

### 완료 작업
- [x] **[Frontend] 용지별 라인 수 동적 계산 및 페이지 청크 분할 (`PrintPreview.jsx`)**:
  - `PrintPreview.jsx` 내부에서 학생 기준 그룹화 및 각 학생의 상담구분 라인 수(`types.length`)를 계산하여 A4 용지 크기에 맞춰 페이지를 가르는 `getGroupedAndPaginatedPages` 페이징 알고리즘을 설계했습니다.
  - 타이틀 영역이 차지하는 면적을 감안해 첫 페이지는 최대 **18 라인**, 이후 페이지는 최대 **22 라인**까지 채우고 넘칠 시 다음 페이지로 이월되도록 한계값을 정밀 튜닝했습니다.
- [x] **[Frontend] 다중 인쇄 페이지 프리뷰 레이아웃 적용 (`PrintPreview.jsx`)**:
  - 쪼개진 각 페이지를 A4 사이즈 카드(`.print-page`)로 반복 렌더링하고, 첫 장(`pageIdx === 0`)에만 대장 타이틀 및 조회 정보 헤더를 보여주도록 분기 처리했습니다.
  - 모든 분할 페이지에 테이블 헤더 컬럼이 완벽히 반복 노출되도록 하여, 종이가 넘어가더라도 열의 정체성을 한눈에 파악할 수 있도록 리팩토링했습니다.
- [x] **[Frontend] 페이징 간 순번 연속성(Index Offset) 및 단일 책임 리팩토링 (`PrintRegisterTable.jsx`)**:
  - `PrintRegisterTable.jsx` 컴포넌트 내부에 복잡하게 얽혀 있던 데이터 그룹화 기능을 `PrintPreview.jsx`로 일원화하고, 테이블 컴포넌트는 단일 페이징 배열만 렌더링하는 순수 프레젠테이션 역할로 역할을 엄격히 분리(SoC)했습니다.
  - 부모로부터 `startIndex` 값을 인계받아 순번 칸에 `{startIndex + idx + 1}`을 대입하여 페이지 전환 시에도 순번이 흐트러짐 없이 순차적으로 카운팅되도록 보정했습니다.
...
## [2026-05-28] 대장(목록) 양식 '조회 기간' 및 '정렬 기준' 메타 헤더 삭제

### 완료 작업
- [x] **[Frontend] 메타 정보 제거 및 마진 보정 (`PrintPreview.jsx`)**:
  - 사용자 피드백을 수용하여 대장 양식 타이틀 하단에 출력되던 `"조회 기간: ... | 정렬 기준: ..."` 메타텍스트 줄을 완전히 제거하여 지면을 정돈했습니다.
  - 서브 헤더 삭제로 좁아진 공간을 메우기 위해 타이틀(`h1`)의 하단 마진 클래스를 기존 `mb-1`에서 `mb-6`으로 보정하여 테이블과 제목 사이의 심미적 안정 거리를 그대로 보존했습니다.
...
## [2026-05-28] 대장(목록) 양식 페이지당 수용 라인 수 임계치 상향 조정

### 완료 작업
- [x] **[Frontend] 페이지 수용력 최적화 및 불필요한 공백 최소화 (`PrintPreview.jsx`)**:
  - 기존의 페이징 용량(1페이지 18라인, 이후 22라인)이 실제 A4 지면 수용도 대비 너무 보수적이어서 하단에 과도한 여백이 남는 현상을 발견하고 이를 튜닝했습니다.
  - 1페이지 최대 라인을 **24 라인**, 2페이지 이후의 최대 라인을 **28 라인**으로 각각 상향하여 A4 용지 지면을 보다 꽉 차고 조화롭게 활용하도록 개선했습니다.
  - 이로써 대장 인쇄 페이지가 불필요하게 쪼개지지 않고 데이터가 밀도 높게 인쇄되도록 가독성을 극대화했습니다.
...
## [2026-05-30] 버전 0.2.21 정식 릴리즈 배포

### 완료 작업
- [x] **[또래상담] 또래상담 대장 분리 및 인쇄 기능 구현 (`PeerCounseling.jsx` 신규 생성 외)**:
  - 또래상담 전용 대장(황색 테마, 목록/입력/수정/삭제) 메뉴 및 화면을 집단상담에서 완전히 분리 신설.
  - 또래상담 등록 버튼에 마법사(도우미) 기능을 통합하고, 에디터 화면 내 불필요해진 '또래상담 도우미' 중복 진입 버튼을 제거하여 UX 최적화.
  - 또래상담 인쇄 시, 등록된 또래상담자(`peer_counsel_students.json`) 명단을 기반으로 총 참여 횟수/명단 등 인쇄 통계를 자동 집계하여 출력하는 대장 인쇄 기능 구현.
  - 또래상담 세션의 세부내용 출력 시 날짜 오름차순 정렬 및 항목 앞에 `- ` 구분자를 추가하여 가독성 강화.
- [x] **[Config] package.json 버전 상향 (`package.json`)**:
  - `electron/package.json` 및 `frontend/package.json` 의 버전 번호를 `0.2.20`에서 **`0.2.21`**으로 상향했습니다.
- [x] **[Git] 릴리즈 태그 발행 (`git tag`)**:
