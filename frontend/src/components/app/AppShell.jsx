...
export default function AppShell() {
  ...
  const { sidebarWidth, editorWidth, setResizing: setResizeTarget } = useLayoutResize()

  // 윈도우 레벨에서 파일 드래그 기본 동작 방어 및 오버레이 활성화 (Drag Counter 방식)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* 사이드바 크기 조절 핸들 */}
      <div
        onMouseDown={(e) => { e.preventDefault(); setResizeTarget('sidebar') }}
      ...
      <div className="flex flex-1 min-w-0">
        ...
        {editorOpen && (
          <div
            onMouseDown={(e) => { e.preventDefault(); setResizeTarget('editor') }}
            className="w-1 hover:w-1.5 cursor-col-resize transition-all z-20 shrink-0 select-none"
            style={{
